...
if st.sidebar.button('🔔 Recommend'):
    with st.spinner('Generating your recommendations... 🚀'):
        ...
        # Create clickable links for Product IDs
        def make_clickable(product_id):
            # Replace with your actual app02 URL
            url = f"http://localhost:8502/?product_id={product_id}"
            return f'<a href="{url}" target="_blank">{product_id}</a>'

        recommendations['Product ID'] = recommendations['Product ID'].apply(make_clickable)
